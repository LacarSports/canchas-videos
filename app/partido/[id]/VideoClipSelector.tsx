...
export default function VideoClipSelector({
  src,
  title,
  videoUrl,
  partidoId,
  ...
  /* ── playback speed + image adjustments ── */
  const SPEEDS = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 4, 6] as const;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === " ") {
        ...
        e.preventDefault();
        setShowImageSettings((s) => !s);
      }
    };
    // capture:true → dispara antes que cualquier handler de elemento (botones, etc.)
  ...
  function puedeActuar() {
    return etiqueta !== null && (etiqueta !== "Otro" || etiquetaCustom.trim() !== "");
  }
  ...
  function handleDetenerGrabacion() {
    ...
    const scrollToPanel = () => {
      setTimeout(() => {
        controlPanelRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" });
      }, 80);
    };
  ...
  function resetToIdle() {
    setEstado("idle");
    setInicioSeg(null); setFinSeg(null); setError(null);
    setFase(null); setProgreso(0);
    setEtiqueta(null); setEtiquetaCustom("");
    setGuardado(false); setCropParams(null);
    resetView(); stopDrag();
    if (blobUrlRef.current) { URL.revokeObjectURL(blobUrlRef.current); blobUrlRef.current = null; }
  }
  ...
  function handleGuardarDestacado() {
    ...
    setGuardado(true);
    // Auto-volver a idle después de mostrar el check 1.5 s
  ...
  async function handleDescargar() {
    ...
    try {
      ...
      a.href = url;
      a.download = `clip_lacar_sports_${Math.floor(inicioSeg)}s-${Math.floor(finSeg)}s.mp4`;
      a.click();
  ...
  function handleWhatsApp() {
    const et = getEtiquetaFinal();
    window.open(`https://wa.me/?text=${encodeURIComponent(`Mirá esta jugada${et ? ` (${et})` : ""}: ${getShareUrl()}`)}`, "_blank");
  }
  ...
  const labelFase: Record<FaseDescarga, string> = {
    ...
    procesando: `${[
      cropParams ? "zoom" : null,
      playbackSpeed !== 1 ? `${playbackSpeed}×` : null,
      (brightness !== 100 || contrast !== 100) ? "imagen" : null,
    ].filter(Boolean).join(" + ") || "clip"}...${progreso > 0 ? ` ${progreso}%` : ""}`,
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ref={containerRef}
        tabIndex={0}
        className="relative overflow-hidden rounded-xl bg-black select-none outline-none"
        style={{
      ...
      >
        <video
          ...
          onTimeUpdate={() => {
            const v = videoRef.current;
            if (v && !seekingRef.current) setCurrentTime(v.currentTime);
          }}
          onLoadedMetadata={() => {
            const v = videoRef.current;
        ...
        {/* ── Speed badge (zoom pill moved to controls bar) ── */}
        {playbackSpeed !== 1 && (
          <div className="absolute top-3 right-3 pointer-events-none z-10">
            <div className="bg-crystal-400/80 backdrop-blur-sm rounded-full px-2.5 py-1 text-white text-xs font-mono">
              {playbackSpeed}×
            </div>
          </div>
        )}
          ...
          <div className="absolute left-0 inset-y-0 w-2/5 flex flex-col items-center justify-center gap-2 pointer-events-none z-20">
            <div className="bg-white/15 rounded-full w-14 h-14 flex items-center justify-center backdrop-blur-sm">
              <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                <path d="M6 6l2 2-4 4 4 4-2 2-6-6zm7 0l2 2-4 4 4 4-2 2-6-6z" />
              </svg>
            </div>
        ...
        {/* ── Recording overlay (always inside video) ── */}
        ...
        {!isGrabando && (
          <div
            className={`absolute bottom-14 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-sm rounded-full px-4 py-1.5 text-white/40 text-xs pointer-events-none whitespace-nowrap transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}
          >
            {zoom <= 1
              ? "Scroll para hacer zoom · Clic y arrastra para mover"
        ...
        {!isGrabando && (
          <div
            className={`absolute inset-x-0 bottom-0 transition-opacity duration-300 ${showControls || !isPlaying || showImageSettings ? "opacity-100" : "opacity-0 pointer-events-none"}`}
          >
            {/* gradient bg */}
            ...
            <div className="relative px-3 pb-2.5 pt-8 space-y-1">
              ...
              <div className="w-full h-1 bg-white/25 rounded-full relative pointer-events-none">
                ...
                {clips && duration > 0 && clips.map((clip) => {
                  ...
                  return (
                    <div
                      key={clip.id}
                      className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 z-20 group/mark pointer-events-auto"
                      style={{ left: `${pctMark}%` }}
                      ...
                      onClick={(e) => {
                        ...
                        const v = videoRef.current;
                        if (!v) return;
                        v.currentTime = clip.inicioSeg;
                        setCurrentTime(clip.inicioSeg);
                      }}
                    >
                      {/* Tooltip */}
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center text-white/60 hover:text-white transition-colors rounded-lg hover:bg-white/10 text-xs font-semibold"
                  title="Retroceder 5s (←)"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/>
                    <text x="8" y="14" fontSize="6" fontWeight="bold" fill="currentColor">5</text>
                  </svg>
                </button>
                ...
                {/* Time */}
                <span className="text-white/55 text-xs font-mono ml-1 tabular-nums">
                ...
                {zoom > 1.05 && (
                  <button
                    onClick={(e) => { e.stopPropagation(); setZoom(1); setPanX(0); setPanY(0); zoomRef.current = 1; panXRef.current = 0; panYRef.current = 0; }}
                    className="text-[10px] text-crystal-400 font-mono border border-crystal-400/30 rounded-full px-2 py-0.5 hover:bg-crystal-400/10 transition-colors"
                    title="Resetear zoom"
                  >
                    {zoom.toFixed(1)}× ✕
                  </button>
                ...
                >
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-200 ${showVolumeSlider ? "w-16 opacity-100" : "w-0 opacity-0"}`}
                  >
                    <input
                      type="range"
                      min={0}
                      ...
                      className="w-16 accent-crystal-400 h-1 rounded-full cursor-pointer"
                    />
                  </div>
                </div>
      ...
      <div ref={controlPanelRef} className="bg-lake-800/60 rounded-xl border border-crystal-400/8 p-5 space-y-4">
        ...
        {isGrabando && inicioSeg !== null && (
          <div className="flex items-center gap-2.5 px-4 py-2.5 bg-crystal-400/5 border border-crystal-400/12 rounded-lg">
            ...
            <span className="text-crystal-400 text-sm">
              Desde <span className="font-mono">{fmt(inicioSeg, true)}</span>
            </span>
            {zoom > 1.05 && (
              <span className="ml-auto text-xs text-crystal-400/60 font-mono">{zoom.toFixed(1)}× zoom</span>
            )}
          </div>
        )}
        ...
        {(estado === "preview" || estado === "descargando") && inicioSeg !== null && finSeg !== null && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 divide-x divide-crystal-400/8 text-center border border-crystal-400/8 rounded-lg overflow-hidden">
              {[
                { label: "Inicio", val: fmt(inicioSeg) },
                { label: "Fin", val: fmt(finSeg) },
                { label: "Duración", val: fmt(duracionClip!), accent: true },
            ...
            {/* Speed / image adjustments indicator */}
            {(playbackSpeed !== 1 || brightness !== 100 || contrast !== 100) && (
              <div className="flex flex-wrap gap-1.5 text-xs text-crystal-400/70 bg-crystal-400/5 border border-crystal-400/8 rounded-lg px-3 py-2">
                ...
                {contrast !== 100 && (
                  <span className="flex items-center gap-1">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" strokeWidth="2"/><path strokeLinecap="round" strokeWidth="2" d="M12 2a10 10 0 010 20V2z" fill="currentColor" stroke="none"/></svg>
                    Contraste {contrast}%
                  </span>
                )}
            ...
            <div className="space-y-2.5">
              <p className="text-xs font-semibold text-mist-600 uppercase tracking-wide flex items-center gap-1.5">
                Etiqueta tu jugada
                <span className="text-crystal-400 normal-case tracking-normal font-normal">(obligatorio)</span>
              </p>
              <div className="flex flex-wrap gap-2">
                {ETIQUETAS.map((e) => (
                  <button
                    key={e}
                    type="button"
                    onClick={() => { setEtiqueta(e); if (e !== "Otro") setEtiquetaCustom(""); }}
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all disabled:opacity-40 ${
                      etiqueta === e
                        ? "bg-crystal-400/12 border-crystal-400 text-crystal-400"
                        : "border-mist-500/15 text-mist-500 hover:border-mist-500/30 hover:text-snow"
                    }`}
                  >
                    {e}
                  </button>
            ...
            {estado === "descargando" && fase && (
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs text-mist-600">
                  <span>{labelFase[fase]}</span>
                  {progreso > 0 && <span>{progreso}%</span>}
                </div>
                <div className="w-full bg-white/5 rounded-full h-1 overflow-hidden">
                  <div
                    className="bg-crystal-400 h-full rounded-full transition-all duration-300"
                    style={{ width: progreso > 0 ? `${progreso}%` : "45%", animation: progreso === 0 ? "pulse 1.5s ease-in-out infinite" : "none" }}
                  />
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                disabled={estado === "descargando"}
                className="py-2.5 rounded-lg text-xs border border-mist-500/15 text-mist-600 hover:text-snow hover:border-mist-500/30 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Otro Clip
              </button>
              <button
                onClick={handleGuardarDestacado}
                disabled={!puedeActuar() || estado === "descargando" || guardado}
                className={`py-2.5 rounded-lg text-xs border transition-all disabled:cursor-not-allowed flex items-center justify-center gap-1 ${
                  guardado
                    ? "border-crystal-400/40 text-crystal-400 bg-crystal-400/5"
                    : "border-mist-500/15 text-mist-500 hover:text-crystal-400 hover:border-crystal-400/30 disabled:opacity-40"
                }`}
              >
                {guardado ? (
              ...
              <button
                ...
                disabled={!puedeActuar() || estado === "descargando"}
                className="py-2.5 rounded-lg text-xs border border-crystal-400/30 bg-crystal-400/8 text-crystal-400 hover:bg-crystal-400/15 hover:border-crystal-400/50 transition-all disabled:opacity-25 disabled:cursor-not-allowed flex items-center justify-center gap-1"
              >
                {estado === "descargando" ? (
            ...
            {puedeActuar() && estado !== "descargando" && (
              <div className="flex items-center gap-2 pt-1 border-t border-crystal-400/8">
                ...
                <button
                  onClick={() => copiar(getShareUrl())}
                  title={copiado ? "Copiado!" : "Copiar enlace"}
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs border transition-all ${
                    copiado ? "border-crystal-400/50 text-crystal-400 bg-crystal-400/8" : "border-mist-500/15 text-mist-600 hover:border-mist-500/30 hover:text-mist-400"
                  }`}
                >
                  {copiado ? (
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-lg border border-mist-500/15 hover:border-mist-500/30 text-mist-600 hover:text-mist-400 flex items-center justify-center transition-all"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
